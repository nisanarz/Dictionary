 ...
 * 	an element failed)
 * 	MAP_SUCCESS the paired elements had been REPLACED successfully (The key was already in the map)
 * 	MAP_SUCCESS_ITEM_ADDED the paired elements had been ADDED successfully (Created a new key in the map)
 */
 ...
 */
MapKeyElement mapGetFirst(Map map, MapResult* result /* must NEVER be NULL */);
 ...
 * 	MAP_OUT_OF_MEMORY if an allocation failed (Meaning the function for copying
  * 	MAP_SUCCESS - Otherwise.
 */
MapKeyElement mapGetNext(Map map, MapResult* result /* must NEVER be NULL */);
 ...
 */
#define MAP_FOREACH(type,iterator,map, pResult) \
	for(type iterator = (type) mapGetFirst(map, pResult) ; \
		iterator ;\
		iterator = mapGetNext(map, pResult))
